...
class MLManager:
    ...
    def can_start_prediction(self) -> bool:
        ...
        if not self.learning_start_date:
            ...
            if patterns:
                self.learning_start_date = datetime.fromisoformat(
                    patterns[0]["timestamp"]
                )
            else:
                return False

        # Check if learning period is complete
        learning_complete = datetime.now() - self.learning_start_date >= timedelta(
            days=settings.ML_LEARNING_PERIOD_DAYS
        )
    ...
    def train_models(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"ML training completed. Accuracy estimate: {self.model_accuracy:.2f}"
            )
            return True
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "learning_start_date": (
                self.learning_start_date.isoformat()
                if self.learning_start_date
                else None
            ),
            "is_trained": self.is_trained,
            "model_accuracy": self.model_accuracy,
            ...
            "data_points": len(
                self.db.get_user_patterns(settings.ML_LEARNING_PERIOD_DAYS)
            ),
        }

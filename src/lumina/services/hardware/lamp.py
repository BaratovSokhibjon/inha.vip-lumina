...
class LampController:
    ...
    def _on_earthquake_alert(self, earthquakes):
        ...
        self.hardware.blink_leds(
            *settings.EARTHQUAKE_ALERT_COLOR, times=5, interval=0.3
        )
    ...
    def set_brightness(self, brightness: int):
        ...
        brightness = max(
            settings.MIN_BRIGHTNESS, min(settings.MAX_BRIGHTNESS, brightness)
        )
        ...
        while self.running:
            try:
                ...
                if (
                    ...
                    > settings.AUTO_COLOR_CYCLE_INTERVAL
                ):
                    self.cycle_color()
        ...
        try:
            ...
            if state:
                ...
                self.current_color = tuple(
                    state.get("current_color", settings.DEFAULT_COLOR)
                )
                self.current_brightness = state.get(
                    "current_brightness", settings.DEFAULT_BRIGHTNESS
                )

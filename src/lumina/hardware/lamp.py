...
class LampController:
    ...
    def __init__(self, config):
        ...
        self.color_cycle = [
            tuple(self.config["red_color"]),
            tuple(self.config["green_color"]),
            tuple(self.config["blue_color"]),
            tuple(self.config["yellow_color"]),
            tuple(self.config["magenta_color"]),
            tuple(self.config["cyan_color"]),
            tuple(self.config["white_color"]),
        ]
    ...
    def _on_earthquake_alert(self, earthquakes):
        ...
        self.logger.warning(
            f"Earthquake alert: {len(earthquakes)} significant earthquakes"
        )

        # Flash red for earthquake alert
        self.hardware.blink_leds(
            *self.config["earthquake_alert_color"], times=5, interval=0.3
        )
        self.hardware.play_alert_sound(2.0)

        # Log environmental event
        for eq in earthquakes:
            self.db.log_environmental_data(
                "earthquake",
                eq["magnitude"],
                {"place": eq["place"], "time": eq["time"].isoformat()},
            )
    ...
    def _on_air_quality_alert(self, aqi_value, aqi_level):
        ...
        alert_color = self.utils.get_aqi_color(aqi_value, self.config)
        if self.is_on:
        ...
        # Log environmental event
        self.db.log_environmental_data(
            "air_quality", aqi_value, {"aqi_level": aqi_level}
        )
    ...
    def _on_temperature_change(self, temperature):
        ...
        if self.mode == "AUTO" and self.is_on:
            temp_color = self.utils.get_temperature_color(temperature, self.config)
    ...
    def set_brightness(self, brightness: int):
        ...
        brightness = max(
            self.config["brightness_settings"]["min_brightness"], min(self.config["brightness_settings"]["max_brightness"], brightness)
        )
    ...
    def _automation_loop(self):
        ...
        while self.running:
            try:
                ...
                if (
                    self.mode == "AUTO"
                    and self.auto_color_cycling
                    and self.is_on
                    and current_time - last_color_cycle
                    > self.config["auto_mode_settings"]["auto_color_cycle_interval"]
                ):
                    self.cycle_color()
            ...
            if should_adjust and self.mode == "AUTO":
                ...
                if "power" in adjustments:
                    ...
                    if power_state and not self.is_on:
                        self.turn_on()
                        self.logger.info(
                            f"ML turned lamp ON (confidence: {confidence:.2f})"
                        )
                    elif not power_state and self.is_on:
                        self.turn_off()
                        self.logger.info(
                            f"ML turned lamp OFF (confidence: {confidence:.2f})"
                        )
                ...
                if "color" in adjustments and self.is_on:
                    ...
                    self.logger.info(
                        f"ML changed color to {color} (confidence: {confidence:.2f})"
                    )
    ...
    def _save_state(self):
        ...
        try:
            ...
            self.utils.save_json(self.config["system"]["state_file_path"], state)
    ...
    def _load_state(self):
        ...
        try:
            state = self.utils.load_json(self.config["system"]["state_file_path"])

            if state:
                self.is_on = state.get("is_on", False)
                self.current_color = tuple(
                    state.get("current_color", self.config["default_color"])
                )
                self.current_brightness = state.get(
                    "current_brightness", self.config["brightness_settings"]["default_brightness"]
                )

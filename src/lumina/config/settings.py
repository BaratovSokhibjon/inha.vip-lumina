...
class Settings:
    ...
    def __init__(self):
        ...
        self.EARTHQUAKE_MIN_MAGNITUDE = float(
            os.getenv("EARTHQUAKE_MIN_MAGNITUDE", 5.5)
        )
        self.BAD_AIR_THRESHOLD = int(os.getenv("BAD_AIR_THRESHOLD", 100))
        self.COLD_TEMPERATURE_THRESHOLD = int(
            os.getenv("COLD_TEMPERATURE_THRESHOLD", 18)
        )
        self.HOT_TEMPERATURE_THRESHOLD = int(os.getenv("HOT_TEMPERATURE_THRESHOLD", 28))

        # Check Intervals (seconds)
        self.EARTHQUAKE_CHECK_INTERVAL = int(
            os.getenv("EARTHQUAKE_CHECK_INTERVAL", 300)
        )
        self.AIR_QUALITY_CHECK_INTERVAL = int(
            os.getenv("AIR_QUALITY_CHECK_INTERVAL", 600)
        )
        ...
        self.ML_PREDICTION_ACCURACY_THRESHOLD = float(
            os.getenv("ML_PREDICTION_ACCURACY_THRESHOLD", 0.75)
        )
        self.ML_MODEL_UPDATE_INTERVAL = int(os.getenv("ML_MODEL_UPDATE_INTERVAL", 3600))
        self.ML_DATA_COLLECTION_INTERVAL = int(
            os.getenv("ML_DATA_COLLECTION_INTERVAL", 60)
        )

        # File Paths
        self.ML_MODEL_PATH = os.getenv("ML_MODEL_PATH", "models/user_pattern.pkl")
        self.ML_DATA_PATH = os.getenv("ML_DATA_PATH", "data/lumnia.db")
        self.STATE_FILE_PATH = os.getenv("STATE_FILE_PATH", "data/lamp_state.json")
        self.LOG_FILE_PATH = os.getenv("LOG_FILE_PATH", "logs/lumnia.log")
        self.DATABASE_PATH = os.getenv("DATABASE_PATH", "data/lumnia.db")

        # LED Strip Configuration (for display purposes)
        self.LED_STRIP_COUNT = int(os.getenv("LED_STRIP_COUNT", 30))

        # Colors (RGB tuples)
        self.DEFAULT_COLOR = self._parse_color(
            os.getenv("DEFAULT_COLOR_RED", "255"),
            os.getenv("DEFAULT_COLOR_GREEN", "255"),
            os.getenv("DEFAULT_COLOR_BLUE", "255"),
        )

        # Air Quality Colors
        self.AQI_GOOD_COLOR = self._parse_rgb_string(
            os.getenv("AQI_GOOD_COLOR", "0,255,0")
        )
        self.AQI_MODERATE_COLOR = self._parse_rgb_string(
            os.getenv("AQI_MODERATE_COLOR", "255,255,0")
        )
        self.AQI_UNHEALTHY_COLOR = self._parse_rgb_string(
            os.getenv("AQI_UNHEALTHY_COLOR", "255,165,0")
        )
        self.AQI_DANGEROUS_COLOR = self._parse_rgb_string(
            os.getenv("AQI_DANGEROUS_COLOR", "255,0,0")
        )
        ...
        # Alert Colors
        self.EARTHQUAKE_ALERT_COLOR = self._parse_rgb_string(
            os.getenv("EARTHQUAKE_ALERT_COLOR", "255,0,0")
        )
        self.EMERGENCY_COLOR = self._parse_rgb_string(
            os.getenv("EMERGENCY_COLOR", "255,0,255")
        )
        ...
        self.AUTO_BRIGHTNESS_ADJUSTMENT = (
            os.getenv("AUTO_BRIGHTNESS_ADJUSTMENT", "True").lower() == "true"
        )

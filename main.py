    ...
    def start_web_interface(self):
        ...
        try:
            ...
            if self.web_process.poll() is None:
                self.logger.info(
                    f"✓ Web interface started on port {settings.STREAMLIT_PORT}"
                )
                self.logger.info(
                    f"  Access at: http://localhost:{settings.STREAMLIT_PORT}"
                )
                return True
            else:
    ...
    def _health_check(self):
        ...
        try:
            ...
            if sys_info.get("memory_percent", 0) > 80:
                self.logger.warning(
                    f"High memory usage: {sys_info['memory_percent']:.1f}%"
                )

            if sys_info.get("disk_percent", 0) > 90:
                self.logger.warning(
                    f"Low disk space: {sys_info['disk_percent']:.1f}% used"
                )
...
def main():
    ...
    parser.add_argument(
        "--setup", action="store_true", help="Run setup before starting"
    )
